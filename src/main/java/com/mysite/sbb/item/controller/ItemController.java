package com.mysite.sbb.item.controller;
...
public class ItemController {
  ...
  @PostMapping("/")
  public ResponseEntity<ApiResponse> registerBasic(
      @RequestBody ItemRegisterRequest itemRegisterRequest) {
    itemService.registerItem(itemRegisterRequest);

    return ResponseEntity.ok().body(ApiResponse.of(HttpStatus.OK, "성공적으로 입력 되었습니다."));
  }
  ...
  public ResponseEntity<ApiResponse> getAllBasic() {
    ...
    return ResponseEntity.ok().body(ApiResponse.of(HttpStatus.OK, "Basic의 데이터 입니다.", basicList));
  }
  ...
  public ResponseEntity<ApiResponse<BasicResponse.BasicSearchResponse>> searchBoards(
    ...
    BasicResponse.BasicSearchResponse response =
        basicService.searchBasics(cursorId, PageRequest.of(0, size), sort);
    return ResponseEntity.ok()
        .body(ApiResponse.of(HttpStatus.OK, "성공적으로 모든 게시글 정보를 조회하였습니다.", response));
  }
  ...
  public ResponseEntity<ApiResponse> getByUpdateDate(
    ...
    List<Basic> basicList =
        basicService.getBasicByUpdateTime(startDateTime, endDateTime, PageRequest.of(0, 10));
  ...
  public ResponseEntity<ApiResponse> updateBasic(
      @PathVariable("basicId") Long basicId, @RequestBody @Valid BasicRegisterRequest request) {
    basicService.updateBasic(basicId, request);
    return ResponseEntity.ok().body(ApiResponse.of(HttpStatus.OK, "성공적으로 수정하였습니다.", true));
  }

  @DeleteMapping("/{basicId}")
  public ResponseEntity<ApiResponse> deleteBasic(@PathVariable("basicId") Long basicId) {
