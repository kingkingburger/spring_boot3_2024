...
/**
 * 이 클래스는 사용자 인증이 실패했을 때 호출되는 핸들러이다. 사용자의 '인증'에 대해 실패하였을 경우 수행되는 Handler로 실패에 대한 사용자에게 반환값을 구성하여
 * 전달한다.
 */
@Slf4j
@Configuration
...
public class CustomAuthFailureHandler implements AuthenticationFailureHandler {
  ...
  /**
   * 이 메서드는 HTTP 요청, HTTP 응답, 그리고 인증 예외를 인자로 받는다. 인증 예외의 타입에 따라 실패 메시지를 설정하고, 이를 JSON 형태로 클라이언트에
   * 응답한다.
   */
  @Override
