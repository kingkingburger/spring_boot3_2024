...
//
//  /**
//   * 이 메서드는 정적 자원에 대해 보안을 적용하지 않도록 설정한다. 정적 자원은 보통 HTML, CSS, JavaScript, 이미지 파일 등을 의미하며, 이들에 대해
// 보안을
...
//   *   <li>CSRF 방지 기능 비활성화
//   *   <li>CORS 설정 적용
//   *   <li>`/resources/**`, `/main/rootPage` 경로 접근 허용
//   *   <li>나머지 요청은 인증된 사용자만 접근 허용
//   *   <li>헤더의 JWT 토큰을 추출하고 검증하는 `JwtAuthorizationFilter` 적용
//   *   <li>세션을 상태 없이 관리
//   *   <li>로그인 페이지 설정 및 로그인 성공 시 리다이렉트 경로 설정
//   *   <li>사용자 이름과 비밀번호 인증을 위한 `CustomAuthenticationFilter` 적용
//   * </ul>
//   *
...
//      HttpSecurity http, CustomAuthenticationFilter customAuthenticationFilter
...
//
//  /**
//   * 1. 커스텀을 수행한 '인증' 필터로 접근 URL, 데이터 전달방식(form) 등 인증 과정 및 인증 후 처리에 대한 설정을 구성하는 메서드다. 이 메서드는 사용자
// 정의
//   * 인증 필터를 생성한다. 이 필터는 로그인 요청을 처리하고, 인증 성공/실패 핸들러를 설정한다.
//   *
...
//
//  /**
//   * 2. authenticate 의 인증 메서드를 제공하는 매니져로'Provider'의 인터페이스를 의미한다. 이 메서드는 인증 매니저를 생성한다. 인증 매니저는 인증
// 과정을
//   * 처리하는 역할을 한다. 과정: CustomAuthenticationFilter → AuthenticationManager(interface) →
...
//
//  /**
//   * 3. '인증' 제공자로 사용자의 이름과 비밀번호가 요구된다. 이 메서드는 사용자 정의 인증 제공자를 생성한다. 인증 제공자는 사용자 이름과 비밀번호를 사용하여 인증을
//   * 수행한다. 과정: CustomAuthenticationFilter → AuthenticationManager(interface) →
//   * CustomAuthenticationProvider(implements)
//   */
//  @Bean
...
//
//  /**
//   * 4. Spring Security 기반의 사용자의 정보가 맞을 경우 수행이 되며 결과값을 리턴해주는 Handler customLoginSuccessHandler: 이
//   * 메서드는 인증 성공 핸들러를 생성한다. 인증 성공 핸들러는 인증 성공시 수행할 작업을 정의한다.
//   */
//  @Bean
...
//
//  /**
//   * 5. Spring Security 기반의 사용자의 정보가 맞지 않을 경우 수행이 되며 결과값을 리턴해주는 Handler customLoginFailureHandler:
// 이
//   * 메서드는 인증 실패 핸들러를 생성한다. 인증 실패 핸들러는 인증 실패시 수행할 작업을 정의한다.
//   */
//  @Bean
...
//   * 사용자인지 확인한다. 만약 사용자가 "ADMIN" 역할을 가지고 있다면, AuthorizationDecision 객체는 true를 반환하고, 그렇지 않다면 false를
//   * 반환한다.
//   */
