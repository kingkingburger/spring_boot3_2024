...
public class JWTTokenGenerator implements TokenGenerator {
  ...
  //    try {
  //      Jws<Claims> tokenClaim =
  //          Jwts.parserBuilder().setSigningKey(this.signKey).build().parseClaimsJws(token);
  //
  ...
  private Date generateAccessTokenExpiration() {
    return new Date(
        System.currentTimeMillis() + Long.parseLong(tokenProperties.expiration().accessToken()));
  }

  private Date generateRefreshTokenExpiration() {
    return new Date(
        System.currentTimeMillis() + Long.parseLong(tokenProperties.expiration().refreshToken()));
  }
